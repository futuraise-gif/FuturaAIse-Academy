...
export class AssignmentModel {
   ...
   */
  static async create(userId: string, data: CreateAssignmentDTO): Promise<Assignment> {
    // Create reference to top-level assignments collection (for student queries)
    const topLevelAssignmentRef = db.collection('assignments').doc();
    ...
    const assignment: any = {
      ...
      allowed_file_types: data.allowed_file_types ?? [FileType.PDF, FileType.DOC, FileType.DOCX],
   ...
   */
  static async findById(courseId: string, assignmentId: string): Promise<Assignment | null> {
    const doc = await db
      .collection(COURSES_COLLECTION)
      .doc(courseId)
      .collection(ASSIGNMENTS_SUBCOLLECTION)
   ...
   */
  static async findByCourse(courseId: string, filters?: {
  ...
  static async update(
    ...
    const topLevelRef = db.collection('assignments').doc(assignmentId);
   ...
   * Publish assignment
   */
  static async publish(courseId: string, assignmentId: string): Promise<Assignment | null> {
   ...
   * Delete assignment
   */
  static async delete(courseId: string, assignmentId: string): Promise<boolean> {
    try {
      const subcollectionRef = db
        .collection(COURSES_COLLECTION)
        .doc(courseId)
   ...
   */
  private static async updateSubmissionStats(courseId: string, assignmentId: string): Promise<void> {
   ...
   */
  static async getStudentAssignments(userId: string): Promise<{
