...
export class AdminInfoController {
    ...
    try {
      ...
      const students = await Promise.all(
        usersSnapshot.docs.map(async (doc: any) => {
          ...
          const enrolledCourses = enrollmentsSnapshot.docs.map((d: any) => d.data().course_id);

          // Get completed courses
          const completedCourses = enrollmentsSnapshot.docs
            .filter((d: any) => d.data().status === 'completed')
            .map((d: any) => d.data().course_id);
          ...
          billingSnapshot.docs.forEach((doc: any) => {
  ...
  // Get all instructors
  static async getAllInstructors(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.user || !AdminInfoController.hasAdminAccess(req.user.role)) {
        res.status(403).json({ message: 'Access denied. Admin privileges required.' });
        return;
      ...
      const instructors = await Promise.all(
        usersSnapshot.docs.map(async (doc: any) => {
          ...
          const activeCourses = coursesSnapshot.docs.filter(
            (d: any) => d.data().status === 'published' || d.data().status === 'active'
          ).length;
          ...
          for (const courseDoc of coursesSnapshot.docs) {
            ...
            reviewsSnapshot.docs.forEach((reviewDoc: any) => {
  ...
  static async getStudentById(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const enrollments = await Promise.all(
        enrollmentsSnapshot.docs.map(async (enrollDoc: any) => {
      ...
      const billingHistory = billingSnapshot.docs.map((doc: any) => ({
        id: doc.id,
        ...doc.data(),
      }));
  ...
  static async getInstructorById(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.user || !AdminInfoController.hasAdminAccess(req.user.role)) {
        res.status(403).json({ message: 'Access denied. Admin privileges required.' });
        return;
      ...
      const courses = await Promise.all(
        coursesSnapshot.docs.map(async (courseDoc: any) => {
          const courseData = courseDoc.data();
