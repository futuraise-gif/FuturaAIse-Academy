...
const webrtcClassesCollection = db.collection('webrtc_classes');
const coursesCollection = db.collection('courses');

// Create a new WebRTC class
export const createWebRTCClass = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const webrtcClass = {
      ...
      scheduled_at: Timestamp.fromDate(new Date(scheduled_at)),
...
export const getInstructorWebRTCClasses = async (req: AuthRequest, res: Response) => {
  try {
    const instructorId = req.user?.userId;
...
export const getStudentWebRTCClasses = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const activeClasses = classes.filter((c: any) => c.status === 'scheduled' || c.status === 'live');
...
export const getAllWebRTCClasses = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (userRole === 'student') {
      ...
      return getStudentWebRTCClasses(req, res);
    }

    if (userRole === 'instructor') {
      console.log('Routing to getInstructorWebRTCClasses');
      return getInstructorWebRTCClasses(req, res);
    }
...
export const startWebRTCClass = async (req: AuthRequest, res: Response) => {
  try {
...
export const endWebRTCClass = async (req: AuthRequest, res: Response) => {
  try {
    const { classId } = req.params;
    const instructorId = req.user?.userId;
